...
export const useToolStore = defineStore('tool', () => {
  ...
  function pauseAccount(id: string, now: number): boolean {
    ...
    settleRunningAccount(account, now)
  ...
  function waitAccount(id: string, now: number): boolean {
    ...
    account.waitingUntil = now + WAIT_DURATION_MS
    return true
  }
  ...
  function resetDailyProgress(now: number): void {
  ...
  function getEffectiveElapsedMs(account: Account, now: number): number {
    ...
    return account.accumulatedMs + Math.max(0, now - account.startedAt)
  }
  ...
  function persistState(): SaveResult {
    const result = savePersistedState(toPersistedState())
