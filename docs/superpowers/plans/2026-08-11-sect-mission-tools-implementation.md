...
- 产出命名路由 `home`、`sect-mission`，供任务 3B 的导航外壳消费。
...
- 消费任务 3A 的 `router`、命名路由 `home` 和 `sect-mission`。
- 产出带响应式导航的可浏览应用框架。
...
| 空仓库一次变更过大难以回退 | 中 | 按八个可验证任务提交，每个核心切片都有独立保存点 |
